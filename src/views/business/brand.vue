    ...
    <div class="bra_content">
        ...
        <div class="bra_bl">
          <div class="bra_ul">
            <a
              ...
              :key="e.id"
              href="javascript:;"
              :class="{bra_li:true,w1:!i,cur:(le==e)}"
              @click="leFilters(e)"
            >{{ e }}</a>
          </div>
        </div>
      ...
      <div class="bra_conBlock">
        <div class="bra_floorUl">
          <div
            :class="{bra_floorLi:true,fir_1:true,cur:fl < 0}"
            @click="floorFilters(-1)"
          >
            <div class="bra_center ">
              全部楼层
            </div>
          </div>
          <div
            v-for="(f,i) in flDa"
            :key="f.id"
            :class="{bra_floorLi:true,cur:i==fl}"
            @click="floorFilters(i)"
          >
            <div class="bra_center ">
              {{ f.title }}
            </div>
          ...
          <div class="bra_ulBl">
            <template v-for="s in store">
              ...
              >
                ...
                <router-link
                  :to="{path:'/business/',query:{sid:s.id,pId:s.pId}}"
                  class="bra_lo"
                >
                  {{ s.store }}
                </router-link>
